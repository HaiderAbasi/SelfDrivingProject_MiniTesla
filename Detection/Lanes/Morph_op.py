...
def RetClosestContour2(gray):
    Outer_Points_list=[]
    ClosestContour_Found = False
    thresh=np.zeros(gray.shape,dtype=gray.dtype)
    ...
    Row,Bot_Row = FindExtremas(thresh)

    #cv2.namedWindow("thresh",cv2.WINDOW_NORMAL)
    #cv2.imshow("thresh",thresh)
    DetectedContourROI = ROI_extracter(thresh,(0, Row + 25),(thresh.shape[1],Bot_Row-15))
    #cv2.namedWindow("DetectedContourROI",cv2.WINDOW_NORMAL)
    #cv2.imshow("DetectedContourROI",DetectedContourROI)

    cnts2 = cv2.findContours(DetectedContourROI, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)[1]
    val=0
    LowRow_a=-1
    LowRow_b=-1
...
def BWContourOpen_speed(BW,MaxDistance):
    ...
    for index, cnt in enumerate(cnts):
        ...
        if (type(BstCentroid_a)!=int):
            ...
            cv2.line(BW_zero,BstCentroid_a,BstCentroid_b,(0,0,255),thickness=2)
