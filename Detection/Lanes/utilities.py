...
def average_2b(Edge_ROI):

    Lane_detected = np.zeros_like(Edge_ROI)
    col = Edge_ROI.shape[1]

    #Edge_Binary = Edge_ROI > 0
    Edge_Binary_nz_pix = np.where(Edge_ROI > 0)

    if(Edge_Binary_nz_pix[0].shape[0]):
        #y = Edge_Binary_nz_pix[0]
        #x = Edge_Binary_nz_pix[1]
        Zpoly = np.polyfit(Edge_Binary_nz_pix[1], Edge_Binary_nz_pix[0], 2)
