                    ...
                    if(detected_sign.shape[1] and detected_sign.shape[0]):
                        ...
                        if write_data:
                            if  (sign =="speed_sign_70"):
                                ...
                                class_id ="3/"
                            img_dir = os.path.abspath("SW Modular Scripts/Detection/Signs/datasets/") + class_id
...
def detect_Signs(frame,frame_draw):
    ...
    if not model_loaded:
        ...
        # 1. Load CNN model
        global model
        model = load_model(os.path.abspath('SW Modular Scripts/data/saved_model.h5'),compile=False)
